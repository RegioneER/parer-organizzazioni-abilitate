...
import static it.eng.parer.org.abilitate.beans.utils.Costants.COD_ERR_BADREQ;
...
public class ConstraintViolationExceptionMapperProvider
	...
	exception.getConstraintViolations().forEach(c -> {
	    ...
	    // case : AppNameQuery not valid
	    if (c.getLeafBean() instanceof AppNameQuery) {
		message.append("?appName=<valore non accettato>");
		message.append(" - ");
