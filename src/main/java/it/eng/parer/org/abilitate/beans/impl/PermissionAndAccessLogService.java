...
import static it.eng.parer.org.abilitate.beans.utils.Costants.IDP_LOG_ACESSI_FUNCT;
import static it.eng.parer.org.abilitate.beans.utils.Costants.IDP_MAX_GIORNI;
import static it.eng.parer.org.abilitate.beans.utils.Costants.IDP_MAX_TENTATIVI_FALLITI;
import static it.eng.parer.org.abilitate.beans.utils.Costants.IDP_QRY_DISABLE_USER;
import static it.eng.parer.org.abilitate.beans.utils.Costants.IDP_QRY_REGISTRA_EVENTO_UTENTE;
import static it.eng.parer.org.abilitate.beans.utils.Costants.IDP_QRY_VERIFICA_DISATTIVAZIONE_UTENTE;
import static it.eng.parer.org.abilitate.beans.utils.Costants.SERVICE_NAME;
import static it.eng.parer.org.abilitate.beans.utils.DateUtilsConverter.convert;
...
import it.eng.parer.org.abilitate.beans.IPermissionAndAccessLogDao;
import it.eng.parer.org.abilitate.beans.IPermissionAndAccessLogService;
...
public class PermissionAndAccessLogService implements IPermissionAndAccessLogService {
	...
	log.atDebug().log("OrganizzazioniAbilitate - Init PermissionAndAccessLogService start");
	...
	log.atDebug().log("OrganizzazioniAbilitate - Init PermissionAndAccessLogService complete");
    }
	...
	try {
	    log.atDebug().log(
		    "OrganizzazioniAbilitate - Verifica abilitazione utente {} al servizio {}",
	...
	try {
	    ...
	    logDto.setNmAttore("Organizzazioni Abilitate WS");
	    ...
	    log.atDebug().log("OrganizzazioniAbilitate - Creazione access log {}", logDto);
	    // insert access log
	...
	if (ObjectUtils.isNotEmpty(params.get(IDP_QRY_REGISTRA_EVENTO_UTENTE))
		...
		log.atDebug().log("OrganizzazioniAbilitate - Creazione access log completata",
			logDto);
